...
class Daily2WeekData(object):
    ...
    def get_data_accu(self):
        ...
        cols = ['date','code','open','high','low','close_hfq','pct_chg','vol','amount']
        df = df[cols].copy()
    ...
    def daily2week(self,path):
        ...
        for code in self.codes:
            ...
            df_bar = df_t.resample('W',label='left', closed='right').agg({'close_hfq':['max', 'min', 'mean', 'first', 'last'],
                                                                'pct_chg':['max', 'min', 'mean', 'first', 'last'],'vol':['sum'],'amount':['sum']
                                                     }                                 
                                                   ).ffill()
            cols = [('close_hfq','max'),('close_hfq','first'),('close_hfq','min'),('close_hfq','last'),('vol','sum'),('amount','sum'),('pct_chg','last')]
            df_bar = df_bar[cols]
        ...
        codes = list(df['code'].unique())
        df.to_csv('/home/rufus/quant/data/ETF_weekly/' + path) #把周线数据存到指定的目录
...
if __name__ == '__main__': 
    ...
    file_list =  os.listdir('/home/rufus/quant/data/ETF') #获取文件列表
    file_l = ['/home/rufus/quant/data/ETF/' + i for i in file_list] #拼接文件列表
    
    now_date = dt.now().strftime('%Y%m%d')
    file_path = 'weekly_ETF_hfq'+ now_date +'.csv'   #拼接文件名
